...
/** User-turn prompt for an initial draft from a structured encounter description. */
export function buildGenerationUserPrompt(description: string, libraryReference?: string): string {
  const reference = libraryReference?.trim()
    ? `\n\nExisting content you may reuse (prefer these exact names over inventing near-duplicates; skip anything irrelevant):\n${libraryReference.trim()}`
    : '';
